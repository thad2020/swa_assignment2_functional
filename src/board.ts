...
export function create<T>(generator: Generator<T>, width: number, height: number): Board<T> {
    const Board: Board<T> = ({
        ...
        height: height,
        generator: generator,
        board: generate(width, height, generator) , 
        positions: positionsInit(width, height)
        // positionsArray: []
    })
...
export function generate<T>( width: number, height: number, generator: Generator<T>) {
    let array = [];
...
export function positionsInit<T>(width: number, height: number): Position[] {
    let array = [];
